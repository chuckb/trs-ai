...
| Script | Role |
|--------|------|
| [`appliance/scripts/check-deps.sh`](../appliance/scripts/check-deps.sh) | Requires `mkosi`, `qemu-system-x86_64`, `guestfish`, `systemd-repart`, and `mbasic/mbasic` |
| [`appliance/scripts/build-image.sh`](../appliance/scripts/build-image.sh) | Builds `trs-ai-basic-m1.raw` |
...
**QEMU / Konsole:** [`mkosi.conf`](../appliance/mkosi/mkosi.conf) sets **`[Runtime] Firmware=linux`** so `mkosi vm` uses **direct kernel boot** (`trs-ai-basic-m1.vmlinuz` + `.initrd`) instead of relying on in-VM UEFI scanning the ESP. The UEFI-only path often hits OVMF errors such as `BdsDxe: failed to load Boot0002 "UEFI Misc Device" … Not Found` when NVRAM boot entries do not match the emulated PCI topology.
...
- **Podman/Docker** — only if you choose containerized builds for reproducibility (see below).
